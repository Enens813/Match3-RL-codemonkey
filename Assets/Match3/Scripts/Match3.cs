...
public class Match3 : MonoBehaviour {
    ...
    public void SwapGridPositions(int startX, int startY, int endX, int endY) {
        ...
        if (startGemGridPosition == null || endGemGridPosition == null) {
            Debug.LogError($"Null Grid Position: start ({startX}, {startY}) = {startGemGridPosition}, end ({endX}, {endY}) = {endGemGridPosition}");
            return;
        }
        ...
        if (startGemGrid == null || endGemGrid == null) {
            Debug.LogError($"Null Gem in Grid: start ({startX}, {startY}) = {startGemGrid}, end ({endX}, {endY}) = {endGemGrid}");
            Debug.LogError($"Full Grid Data: {grid}");
            return;
        }
    ...
    public List<PossibleMove> GetAllPossibleMoves() {
        ...
        for (int y = 0; y < gridHeight; y++) {
            for (int x = 0; x < gridWidth; x++) {
                ...
                testPossibleMoveList.Add(new PossibleMove(x, y, x - 1, y + 0)); // left
                testPossibleMoveList.Add(new PossibleMove(x, y, x + 1, y + 0)); // right
                testPossibleMoveList.Add(new PossibleMove(x, y, x + 0, y + 1)); // up
                testPossibleMoveList.Add(new PossibleMove(x, y, x + 0, y - 1)); // down
    ...
    public GemSO GetGemSO(int x, int y) {
    ...
    public bool HasGlass(int x, int y){
        ...
        if (gemGridPosition.GetGemGrid() == null) return false;

        return gemGridPosition.HasGlass();

    }

    public bool HasGemAtPosition(int x, int y) {
        if (!IsValidPosition(x, y)) return false;
        
        GemGridPosition gemGridPosition = grid.GetGridObject(x, y);
        return gemGridPosition != null && gemGridPosition.GetGemGrid() != null;
    }

    
    public bool TryIsGameOver() {
        Debug.Log("trying gameover");
